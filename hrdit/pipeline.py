...
from .hap import prepare_hap_masks, set_hap
from .spa import set_spa, set_spa_filter
...
class FluxPipeline(
    ...
    def __call__(
        ...
        scope_plan = None,
        #########################
        # flex-HAP / SPA scheduling (all optional; None keeps legacy behavior)
        hap_step_ranges = None,   # per stage, [start, end) of the steps that use HAP
        hap_anchor_stride = 0,    # every stride-th 64-token image block is globally visible (0 = off)
        spa_step_ranges = None,   # per stage, [start, end) of the steps that use SPA
        group_nums = None,        # per upscale_step SPA bundle grid size (e.g. [80, 80]); required for SPA
        spa_block_ids = None,     # [double_ids, single_ids] restricting SPA to those blocks; None = all
        #########################
        #########################
        save_dir = None,
        img_index = None,
        ...
        save_folder = save_dir + "_res2k/"
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)

        if len(target_heights) > 2:
            save_folder_4k = save_dir + "_res4k/"
            if not os.path.exists(save_folder_4k):
                os.makedirs(save_folder_4k)
        ...
        height_dict = {}
        width_dict = {}
        remove_scope_plan(self.transformer)

        # Reset attention flags left over from a previous pipeline call: processors
        # persist on the transformer across calls, and a stale hap_on/spa_on from the
        # last high-res step of the previous image would fire during this image's 1K
        # base stage (whose seq_len has no prepared BlockMask).
        set_hap(self.transformer, False)
        set_spa(self.transformer, False)
        if spa_block_ids is not None:
            set_spa_filter(self.transformer, spa_block_ids[0], spa_block_ids[1])
        else:
            set_spa_filter(self.transformer, None, None)
        ...
        if enable_dwt and hasattr(self, 'DWT') is False:
        ...
        for upscale_step, (target_height, target_width) in enumerate(zip(target_heights, target_widths)):
            ...
            print(f"### sampling {target_height} x {target_width} ###")

            seq_len_stage = 512 + (target_height // 16) * (target_width // 16)
            hap_rng = None
            if hap_step_ranges is not None:
                hap_rng = list(hap_step_ranges[upscale_step])
                if hap_rng[1] <= 0:  # end <= 0 counts back from the stage's last step
                    hap_rng[1] = num_inference_steps_highres[upscale_step] + hap_rng[1]
            hap_active_stage = (
                scope_plan is not None
                and hap_rng is not None
                and hap_rng[0] < hap_rng[1]
            )
            if hap_active_stage:
                assert not swin_pachify, "flex-HAP masks assume a fixed seq_len; disable swin_pachify"
                prepare_hap_masks(scope_plan, self.transformer, seq_len_stage,
                                  anchor_stride=hap_anchor_stride)
            ...
            if enable_dwt:
                LL, _ = self.DWT(guidance_image.to(torch.float32))
            ...
            with self.progress_bar(total=num_inference_steps_highres[upscale_step]) as progress_bar:
                for i, t in enumerate(dlfg_timesteps):
                    ...
                    spa_on_step = (
                        group_nums is not None
                        and spa_step_ranges is not None
                        and spa_step_ranges[upscale_step][0] <= i < spa_step_ranges[upscale_step][1]
                    )
                    hap_on_step = (
                        hap_active_stage
                        and hap_rng[0] <= i < hap_rng[1]
                    )

                    set_spa(self.transformer, spa_on_step)
                    set_hap(self.transformer, hap_on_step)
                    ...
                    noise_pred = self.transformer(
                            ...
                            group_num=(group_nums[upscale_step] if spa_on_step else None),
                        )[0]
            ...
            if upscale_step == 0:
                self.save_image(latents, target_height, target_width, save_folder + f'{img_index}.png', output_type)
            elif len(target_heights) > 2 and upscale_step == 1:
                self.save_image(latents, target_height, target_width, save_folder_4k + f'{img_index}.png', output_type)
    ...
    def save_image(self, latents, height, width, output_path, output_type, ):
        ...
        base_index = int(basename)
        for idx, img in enumerate(image):
            img.save(os.path.join(dirname, f"{base_index + idx}.png"))
        return 
    ...
    def flowmatch_step_diffhigh(
        ...
        if flow_choice is not None: 
            ...
            if LL_guidance is not None:
                pred_x0 = (pred_x0 / self.vae.config.scaling_factor) + self.vae.config.shift_factor
                pred_x0_image = self.vae.decode(pred_x0.to(self.vae.dtype), return_dict=False)[0]

                _, HH = self.DWT(pred_x0_image.to(torch.float32))
                coeffs = (LL_guidance, HH)
                pred_original_image = self.iDWT(coeffs)
                pred_original_sample, _ = self.encode_vae_latents(pred_original_image, batch_size, num_channels_latents, target_height, target_width, )
            ...
            model_output = (sample - pred_original_sample) / (sigma + 1e-6) if LL_guidance is not None else (sample - original_pred_x0) / (sigma + 1e-6)
            #         else: 
